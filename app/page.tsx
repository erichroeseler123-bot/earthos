  ...
  return (
    <>
      ...
      <section className="mb-24 grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* $59 SHUTTLE - 400x300 (Subtracted 100px from top) */}
        ...
        {/* PRIVATE SUV - 400x300 (Subtracted 100px from top) */}
          ...
          {filteredShows.map((node) => (
            <div key={node.slug} className="p-4 border border-zinc-800 bg-zinc-900/30 flex flex-col md:flex-row items-center gap-6 group hover:border-neon-blue transition-all mb-4">
              <div className="w-48 h-28 overflow-hidden border border-zinc-700 bg-zinc-800 flex-shrink-0 relative">
                {/* SYSTEM RESOLUTION LOGIC */}
                <img 
                   src={`https://source.unsplash.com/featured/800x450/?${encodeURIComponent(node.artist)},concert`} 
                   alt={node.artist} 
                   className="w-full h-full object-cover" 
                   onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=400'; }}
                />
              </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono w-full pb-20">
      <main className="p-8 lg:p-12 max-w-7xl mx-auto">
        <Suspense fallback={<div className="text-neon-blue animate-pulse uppercase tracking-widest">Resolving_Visual_Nodes...</div>}>
          <HomeContent />
        </Suspense>
      </main>
    </div>
